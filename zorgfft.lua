...
local function calculateFactors(n)
	...
	for i = 0, #factors-1 do factorsPtr[i] = factors[1+i] end
...
local function work_t(iRe, iIm, oRe, oIm, oidx, f, factors, fidx, twRe, twIm, fstride, istride, isInverse,
	BDiRe, BDiIm, BDoRe, BDoIm, BDfactors, BDtwRe, BDtwIm) -- ByteData objects passable only through channels...
	local p, m  = factors[fidx], factors[fidx + 1]
	fidx = fidx + 2
...
local function fft(inputRe, inputIm, outputRe, outputIm)
	...
	if type(inputRe) == 'table' then
		...
		for i=0, n-1 do inputRePtr[i] = tempRe[1+i] end
		...
		if not inputIm then
			...
			for i=0, n-1 do inputImPtr[i] = 0.0 end
		else
			...
			inputIm = love.data.newByteData(n * ffi.sizeof('double'))
			inputImPtr = ffi.cast("double *", inputIm:getFFIPointer())
			for i=0, n-1 do inputImPtr[i] = tempIm[1+i] end
		end
	...
	elseif type(inputRe) == 'cdata' then
		...
		assert(ffi.istype(ffi.typeof('double[' .. n .. ']'), inputRe),
		...
		inputRe = love.data.newByteData(n * ffi.sizeof('double'))
		inputRePtr = ffi.cast("double *", inputRe:getFFIPointer())
		for i=0, n-1 do inputRePtr[i] = tempRe[i] end
		...
		if not inputIm then
			...
			assert(ffi.istype(ffi.typeof('double[' .. n .. ']'), inputIm),
				"The passed ffi array wasn't of type double.")
			local tempIm = inputIm
			inputIm = love.data.newByteData(n * ffi.sizeof('double'))
			inputImPtr = ffi.cast("double *", inputIm:getFFIPointer())
			for i=0, n-1 do inputImPtr[i] = tempIm[i] end
		end

	elseif love and inputRe and inputRe.type and inputRe:type() == 'SoundData' then

		assert(inputRe:getChannelCount() == 1,
		...
		inputRe = love.data.newByteData(n * ffi.sizeof('double'))
		inputRePtr = ffi.cast("double *", inputRe:getFFIPointer())
		for i=0, n-1 do inputRePtr[i] = tempRe:getSample(i) end
	...
	assert(outputRe.type and outputRe:type() == 'ByteData',
		"Parameter #3 must be a LöVE ByteData object!")
	assert(outputIm.type and outputIm:type() == 'ByteData',
		"Parameter #4 must be a LöVE ByteData object!")
	assert(outputRe:getSize() / ffi.sizeof('double') == n,
		"Parameter #3's size isn't correct; must be the same as all other parameters.")
	assert(outputIm:getSize() / ffi.sizeof('double') == n,
...
local function fft_t(inputRe, inputIm, outputRe, outputIm)
	...
	work_t(inputRePtr, inputImPtr,
		...
		factorStride, inputStride,
		false,
		inputRe, inputIm, outputRe, outputIm, factors, twiddlesRe, twiddlesIm) -- ByteData objects
...
local function setupThreads(tc)
	...
	for i = 0, threadCount-1 do
		...
		toThread[i]:performAtomic(function(ch)
			ch:push('procThread?')
			ch:push(fromThread)
		end)
